    ...
    <form action="{{ route('admin.profile.update') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm p-6 space-y-6">
        ...
        <div class="flex items-center gap-6">
            @if(auth()->user()->avatar_path && file_exists(public_path('storage/' . auth()->user()->avatar_path)))
                <img src="{{ asset('storage/' . auth()->user()->avatar_path) }}" alt="Profile Photo" class="w-24 h-24 rounded-full object-cover border-4 border-gray-200" id="avatar-preview">
            @else
                <div class="w-24 h-24 rounded-full bg-green-600 flex items-center justify-center border-4 border-gray-200" id="avatar-preview">
            ...
            <div class="flex-1">
                ...
                <input type="file" name="avatar" id="avatar" accept="image/*" class="w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-green-50 file:text-green-700 hover:file:bg-green-100" onchange="previewAvatar(event)">
        ...
        <div class="pt-6 border-t border-gray-200">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Your unique admin identifier (cannot be changed)</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Role</label>
                    <input type="text" value="@if(auth()->user()->role === 'super_admin')Super Administrator @elseif(auth()->user()->role === 'academic_admin')Academic Administrator @elseif(auth()->user()->role === 'registrar_admin')Registrar Administrator @elseif(auth()->user()->role === 'technical_admin')Technical Administrator @endif" 
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50" disabled>
                    <p class="text-xs text-gray-500 mt-1">Contact Super Admin to change your role</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex gap-4 pt-4">
            ...
            <a href="{{ route('admin.profile.index') }}" class="px-6 py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium transition">
                Cancel
            </a>
        </div>
    </form>
