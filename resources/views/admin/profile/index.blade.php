...
@section('toolbar')
    <div class="flex items-center justify-end gap-3 w-full">
        <a href="{{ route('admin.profile.edit') }}" class="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition">
            ...
            <div class="flex items-center space-x-6">
                ...
                @else
                    <div class="w-24 h-24 rounded-full bg-white flex items-center justify-center border-4 border-white shadow-lg">
                        <span class="text-primary-600 text-2xl font-bold">{{ strtoupper(substr($admin->first_name, 0, 1)) }}{{ strtoupper(substr($admin->last_name, 0, 1)) }}</span>
                    </div>
                @endif
                <div class="text-white">
                    <h2 class="text-2xl font-bold">{{ $admin->full_name }}</h2>
                    <p class="text-green-100 mt-1" style="text-transform: capitalize;">
        ...
        <div class="p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="text-sm font-medium text-gray-600">Login ID / Username</label>
                        <p class="text-gray-900 mt-1">{{ $admin->login_id ?: 'N/A' }}</p>
                    </div>

                    <div>
                        <label class="text-sm font-medium text-gray-600">Account Status</label>
                        <p class="mt-1">
                            <span class="text-white px-3 py-1 {{ $admin->is_active ? 'bg-primary-700' : 'bg-red-500' }} rounded-full text-sm">
        ...
        <div class="flex">
            ...
            <div class="ml-3 text-sm text-blue-800">
                ...
                <ul class="mt-2 space-y-1 list-disc list-inside">
                    <li>You can update your email, profile photo, and password.</li>
                    <li>Official information (name, role, system account) is restricted.</li>
                </ul>
            </div>
        </div>
    </div>
