...
@section('toolbar')
    <div class="flex items-center justify-end gap-3 w-full">
        <a href="{{ route('teacher.profile.index') }}" class="inline-flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2.5 rounded-lg text-sm font-medium transition">
...
<div class="p-6 max-w-4xl">
    ...
    <!-- Edit Profile Form -->
    <form action="{{ route('teacher.profile.update') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="pt-6 border-t border-gray-200">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Your area of expertise or teaching focus</p>
                </div>

                @if($teacher->teacherProfile && $teacher->teacherProfile->department)
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Department</label>
                    <input type="text" value="{{ $teacher->teacherProfile->department }}" 
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50" disabled>
                    <p class="text-xs text-gray-500 mt-1">Managed by Academic Admin - contact them to update</p>
                </div>
                @endif
            </div>
        </div>
    ...
    <form action="{{ route('teacher.profile.password') }}" method="POST" class="bg-white rounded-xl shadow-sm p-6 space-y-6 mt-6">
        @csrf
        @method('PUT')
